...
const Navbar = {
        ...
        <div class="flex items-center justify-between h-16 px-4 lg:px-6">
          ...
          <div class="flex items-center space-x-3">
            ...
            <div class="relative" id="user-menu">
              <button onclick="Navbar.toggleUserMenu()"
                ...
                <svg class="hidden md:block w-4 h-4 text-gray-400 transition-transform duration-200" id="user-menu-chevron" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                </svg>
              </button>

              <!-- Dropdown -->
              <div id="user-dropdown" class="hidden absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-50">
                <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                  <p class="text-sm font-medium text-gray-900 dark:text-white truncate">${Utils.escapeHtml(user.name)}</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400 truncate">${user.email || ""}</p>
                </div>
                <button onclick="Router.go('profile'); Navbar.toggleUserMenu()"
                  class="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center space-x-2">
                  <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                  </svg>
                  <span>${__("nav.profile")}</span>
                </button>
                <button onclick="Router.go('settings'); Navbar.toggleUserMenu()"
                  class="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex items-center space-x-2">
                  <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button onclick="Navbar.logout()"
                  class="w-full text-left px-4 py-2.5 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors flex items-center space-x-2">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/>
                  </svg>
                  <span>${__("action.logout")}</span>
                </button>
              </div>
            </div>
          </div>
  ...
  logout() {
    Modal.confirm(
      __("message.confirmLogout") || "Tem certeza que deseja sair?",
      () => {
        Storage.logout();
        window.location.reload();
      },
