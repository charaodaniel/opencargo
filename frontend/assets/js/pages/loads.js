...
const LoadsPage = {
      ...
      <div class="fade-in">
        <div class="flex items-center justify-between mb-6">
          ...
          <div class="flex items-center space-x-2">
            ${Utils.renderExportCsvButton('LoadsPage')}
            ...
            },
            {
              ...
              render: (r) => `
                <div class="flex items-center space-x-1">
                  <button onclick="LoadsPage.openForm('${r.id}')" class="p-1.5 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors" title="Editar">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                  </button>
                  <button onclick="LoadsPage.attachDocument('${r.id}', '${Utils.escapeHtml(r.title || "")}')" class="p-1.5 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors" title="Anexar documento">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"/></svg>
                  </button>
                  <button onclick="LoadsPage.confirmDelete('${r.id}')" class="p-1.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors" title="Excluir">
  ...
  /**
   * Abre modal para anexar documento a uma carga
   */
  attachDocument(loadId, loadTitle) {
    MatchingPage.attachDocument(loadId, loadTitle);
  },
