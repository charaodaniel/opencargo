...
const FreightsPage = {
  ...
  async _fetchFreights() {
    ...
    try {
      ...
      this._exportConfig.data = this._freights;
    } catch {
      this._freights = [];
    }
  },
    ...
    return `
      <div class="fade-in">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          ${Utils.renderExportCsvButton('FreightsPage')}
        </div>
  ...
  setFilter(key, value) {
    ...
    // Mantem o filterCard sincronizado
    if (key === "status") {
  ...
  /**
   * Define o filtro card (sincroniza com _filters.status)
   */
  setFilterCard(value) {
    this._filterCard = value;
